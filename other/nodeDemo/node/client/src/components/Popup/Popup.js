...
class Popup extends Component {
  ...
  render() {
    let {showPopup,hideModal,applyFee,Balance,payTypeId}=this.props;
    let { MathCeil }= this;
    //计算属性
    let isEnought=(Balance-applyFee)>=0;
    return (
      <WingBlank>
        <WhiteSpace />
        <Modal
        ...
        >
          <List renderHeader={() => <div><span className={"abs l10"} onClick={hideModal}><Icon type={"cross"}/></span> <span className={"c3 fz16"}>确认支付</span></div>}>
            ...
            <List.Item>
              付款类型
              <div className={"fr "}>课程报名</div>
            </List.Item>
            <List.Item>
              使用余额
              <div className={"fr"}>- ¥{isEnought ? applyFee : Balance}</div>
            </List.Item>

            <List.Item>
              还需支付
              <div className={"fr orange"}> ¥{isEnought ? 0 : MathCeil(applyFee-Balance)}</div>
            </List.Item>
            ...
            <Button type="primary" onClick={this.props.buy} className={"bdrs0_button"}>
              {
